...
class _GenerateState extends State<GenerateList> {
  ...
  final List<Event> events = [
    ...
    Event("Nithin", DateTime(0, 9, 12), false, true),
  ...
  Widget _buildRow(i) {
    ...
    return ListTile(
      ...
      trailing: Icon(
        events[i].getIsBirthday() ? Icons.cake : Icons.local_florist,
        color: events[i].getEvenDate().month > month
            ? Colors.red
            : events[i].getEvenDate().month == month
                ? events[i].getEvenDate().day >= date ? Colors.red : Colors.grey
                : Colors.grey,
      ),
      subtitle: Text(
        events[i].getEvenDate().year == 0
            ? events[i].getEvenDate().day.toString() +
                "/" +
                events[i].getEvenDate().month.toString()
            : events[i].getEvenDate().day.toString() +
                "/" +
                events[i].getEvenDate().month.toString() +
                "/" +
                events[i].getEvenDate().year.toString(),
